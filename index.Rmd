...
x<-xy
```
* The years range from `r min(x$Fiscal.Year)` to `r max(x$Fiscal.Year)` and it has `r length((unique(x$Parameter)))` chemicals covering 63 locations in MA and RI
...
```  
* Post loading RI data was at `r nrow(x)` records and `r ncol(x)` variables.  
* Post loading MA data was at `r nrow(y)` records and `r ncol(y)` variables.  
...
xy <- rbind(x,y)
```
* The consolidated data set is `r nrow(xy)` records and `r ncol(xy)` variables.
```{r save_the_data}
...
* Example Plot
```{r echo=FALSE}
g <- ggplot(data=x[x$Parameter=="Mercury" & x$Fiscal.Year==2011,], aes(x=Specific.Location, y=Value)) + 
                                                         geom_bar(stat="identity", width=.5, fill="blue") + 
                                                         xlab("Specific Location - ") + 
                                                         ylab("micrograms per dry gram")+ggtitle("2011 Mercury Levels")+
                                                         theme(plot.title = element_text(lineheight=.8, face="bold"), axis.text.x  = element_text(angle=45, vjust=0.5, size=16))
plot(g)
```
